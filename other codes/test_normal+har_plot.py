...
filename_1 = './data/wm_random123.xyz'
...
for i in range(len(R_ang)):
    ...
    e_har_2.append(float(har_E(np.reshape(c_2[i]*Q_2_bohr_vec, (1,9)), force_3Nx3N)))
    e_har_3.append(float(har_E(np.reshape(c_3[i]*Q_3_bohr_vec, (1,9)), force_3Nx3N)))
...
# file_log = "data/DE_{}".format(base_filename.split("_", 1)[-1])
...
#########################################################################################
x_interp = np.linspace(-0.5,0.8, 100)  # Interpolate between 1 and 10, with 100 points
# Interpolate the y-values at the specified points
y_interp = int_func_1(x_interp)
# Plot the original data and the interpolated curve
plt.figure(figsize=(8, 6))
plt.scatter(x_coeff, E_eV_1, label='Original Data', color='red', marker='o')
...
plt.ylabel('E_eV')
plt.legend()
plt.title('CubicSpline Interpolation_mode_1')
plt.grid(True)
plt.show()
# Interpolate the y-values at the specified points
y_interp = int_func_2(x_interp)
# Plot the original data and the interpolated curve
plt.figure(figsize=(8, 6))
plt.scatter(x_coeff, E_eV_2, label='Original Data', color='red', marker='o')
plt.plot(c_2, e_2,'.b' , label='interpolated data')
plt.plot(c_2, e_har_2,'.', color = 'black' , label='harmonic data')
plt.plot(x_interp, y_interp, label='CubicSpline Interpolation', color='green')
plt.xlabel('coeff.2')
plt.ylabel('E_eV')
plt.legend()
plt.title('CubicSpline Interpolation_mode_2')
plt.grid(True)
plt.show()
# Interpolate the y-values at the specified points
y_interp = int_func_3(x_interp)
# Plot the original data and the interpolated curve
plt.figure(figsize=(8, 6))
plt.scatter(x_coeff, E_eV_3, label='Original Data', color='red', marker='o')
plt.plot(c_3, e_3,'.b' , label='interpolated data')
plt.plot(c_3, e_har_3,'.', color = 'black' , label='harmonic data')
plt.plot(x_interp, y_interp, label='CubicSpline Interpolation', color='green')
plt.xlabel('coeff.3')
plt.ylabel('E_eV')
plt.legend()
plt.title('CubicSpline Interpolation_mode_3')
plt.grid(True)
plt.show()

plt.figure(figsize=(8, 6))
plt.plot(e_sum, e_har,'.b' , label='interpolated data_123 ')
plt.plot(e_sum, e_sum, linestyle='--', label='y=x', color='black')
plt.xlabel('Sum of interpolated energy')
plt.ylabel('Harmonic energy')
plt.legend()
plt.title('int. vs har.')
plt.grid(True)
plt.show()

#coefficient 맞게 나오나 wm_random이용하여 check용
plt.figure(figsize=(8, 6))
plt.plot(c_1, E_input_eV,'.b' , label='coeff.1 check')
plt.xlabel('calculated coeff.1')
plt.ylabel('original coeff.1')
plt.legend()
plt.title('Coeff.1 check')
plt.grid(True)
plt.show()

# modes = [1, 2, 3]
# data = [(c_1, e_1, e_har_1, E_eV_1), (c_2, e_2, e_har_2, E_eV_2), (c_3, e_3, e_har_3, E_eV_3)]

# # Create a single figure with subplots
# fig, axs = plt.subplots(1, len(modes), figsize=(8, 6 * len(modes)))

# for i, mode in enumerate(modes):
#     c_i, e_i, e_har_i, E_eV_i = data[i]

#     # Use the appropriate interpolation function based on the mode
#     if mode == 1:
#         y_interp = int_func_1(x_interp)
#     elif mode == 2:
#         y_interp = int_func_2(x_interp)
...
#     # Create the subplot
#     axs[i].scatter(x_coeff, E_eV_i, label='Original Data', color='red', marker='o')
#     axs[i].plot(c_i, e_i, '.b', label='Interpolated Data')
#     axs[i].plot(c_i, e_har_i, '.', color='black', label='Harmonic Data')
#     axs[i].plot(x_interp, y_interp, label='CubicSpline Interpolation', color='green')
#     axs[i].set_xlabel(f'coeff.{mode}')
#     axs[i].set_ylabel('E_eV')
#     axs[i].legend()
#     axs[i].set_title(f'CubicSpline Interpolation_mode_{mode}')
#     axs[i].grid(True)
